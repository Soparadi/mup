...
function grilleRows(format, buffer) {
  if (format === 'xlsx') return lireXlsx(buffer)
  if (format === 'csv') return parseCSV(buffer.toString('utf8'))
  return null
}
...
function construirePlan(lignes) {
  ...
  for (const l of lignes) {
    ...
    // Indépendant : un SEUL dirigeant et il EST la société (societe_id null).
    ...
    // ── ÉTAPE 4 — routage des coordonnées selon leur type ──
    const telType = classerTelephone(l.tel)
    const emailType = classerEmail(l.email)
    // Le fixe / numéro douteux et l'email générique vont sur la SOCIÉTÉ ;
    // le mobile et l'email nominatif/perso vont sur la 1ère personne.
    const telSociete = l.tel && (telType === 'fixe' || telType === 'douteux')
    const emailSociete = l.email && emailType === 'societe'

    // Société tierce : on prépare le record et on y route les coords entreprise.
    ...
    if (societeTierce) {
      ...
      if (telSociete && !s.tel) s.tel = l.tel
      if (emailSociete && !s.email) s.email = l.email
      for (const k of ['site', 'adresse', 'ville', 'cp']) if (!s[k] && l[k]) s[k] = l[k]
    }

    // Coords destinées à la 1ère personne. Repli : sans société-cible (indépendant
    // ou aucune société), une coordonnée « entreprise » ne se perd pas et reste
    // sur la personne plutôt que d'être jetée.
    const telPremier = (l.tel && (telType === 'mobile' || !societeTierce)) ? l.tel : ''
    const emailPremier = (l.email && (emailType === 'personne' || !societeTierce)) ? l.email : ''
    ...
    gens.forEach((g, idx) => {
      ...
      const statut = (societeTierce || independant || estEmailPro(email) || poste)
        ? 'pro' : 'reserve'
      const personne = {
        civilite: g.civilite || '', prenom: g.prenom, nom: g.nom, poste,
        email, tel, linkedin: estPremier ? l.linkedin : '',
        source: l.source, statut,
        societe_cle: societeTierce ? societeCleFinal : null, action: 'creer'
      }

      // Clé de résolution : email, sinon nom normalisé + clé société.
      const nomNorm = normaliserSociete([g.prenom, g.nom].filter(Boolean).join(' '))
