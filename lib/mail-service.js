...
// Parcourt l'arbre MIME renvoyé par Gmail et rassemble ses parties de corps :
// le texte d'un côté, le HTML de l'autre, chacun rendu tel qu'il est. Ce que
// l'appelant en fait — préférer l'un, retomber sur l'autre — se décide ensuite.
function googleBodyParts(payload) {
  const plain = []
  const html = []
  const walk = (part) => {
    if (!part) return
...
export async function getGoogleMessageBody(db, ownerId, email, messageId) {
  ...
  const h = (name) => (headers.find(x => x.name === name) || {}).value || ''
  const parts = googleBodyParts(data.payload)
  return {
    id: data.id,
    ...
    subject: h('Subject'),
    date: h('Date') || null,
    text: parts.plain || (parts.html ? htmlToText(parts.html) : ''),
    html: parts.html,
    attachments: []
  }
}
