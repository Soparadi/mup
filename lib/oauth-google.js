...
export async function revokeRefreshToken(refreshToken) {
  ...
  try {
    ...
    // Échec rendu visible (RGPD / observabilité) sans changer le contrat :
    // signature et comportement non-throw inchangés — les appelants
    // (/auth/google/disconnect, cron purge deleteUserCascade) comptent sur un
    // retour false, jamais sur une exception. On journalise juste le motif ;
    // la déconnexion / suppression aboutit malgré tout.
    console.warn('[oauth-google:revokeRefreshToken] révocation échouée :', e?.message || e)
    return false
  }
}
