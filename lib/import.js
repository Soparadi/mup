...
function trouverCanon(nh) {
  ...
  // N2(a) — colonnes techniques Odoo (relations *_id/name : source_id/name,
  // industry_id/name, stage_id/name, team_id/name, user_id/name, country_id/id) :
  // ni nom de personne ni société. Neutralisées (jamais routées vers nom_complet).
  if (/\bid (name|id)\b/.test(nh) || /\b(industry|campaign)\b/.test(nh)) return null
...
function calculerColKeys(rows) {
  ...
  // N2(c) — "name" (titre d'opportunité Odoo "Agence Event - X") n'est pas un
  // nom de personne quand une colonne contact/partner name distincte existe :
  // on le neutralise pour qu'il ne pollue plus nom_complet.
  if (hs.some(h => h.includes('contact name') || h.includes('partner name'))) {
    colKeys = colKeys.map((k, i) => (k === 'nom_complet' && hs[i] === 'name') ? null : k)
  }
...
export const DOMAINES_PERSO = new Set([
...
function enrichirPersonne(a, b) {
  for (const k of ['civilite', 'prenom', 'nom', 'poste', 'email', 'tel', 'linkedin', 'note_personne']) {
    if (!a[k] && b[k]) a[k] = b[k]
  }
...
// Exportée : réutilisée par le rapprochement OSM (pont téléphone).
...
// Annotation entre parenthèses d'un dirigeant : vraie FONCTION (-> poste) ou
// simple zone/mention (-> note_personne). Liste explicite de fonctions ; tout le
// reste ("Bretagne", "Val de Loire"…) part en note, jamais en poste.
const STEMS_FONCTION = ['geran', 'cogeran', 'presiden', 'directeu', 'directri', 'dg', 'pdg', 'associ', 'fondat', 'responsab', 'commercial', 'dirigean']
function estFonction(annot) {
  const mots = normHeader(annot).split(' ').filter(Boolean)
  return mots.some(w => STEMS_FONCTION.some(st => w.startsWith(st)))
}
...
function decouperDirigeants(brut, posteCol) {
  ...
  let noteShared = ''
  // N2(d) — plusieurs dirigeants annotés "X (zone/titre) Y (zone/titre)" : une
  // parenthèse INTERNE suivie d'un nouveau nom marque une frontière. On insère
  // un séparateur pour les éclater, chacun gardant ensuite sa propre annotation.
  const txtAvant = txt
  txt = txt.replace(/\)\s+(?=[A-Za-zÀ-ÿ])/g, ') / ')
  const aEclateParen = txt !== txtAvant
  // Poste partagé en fin de cellule UNIQUEMENT sans éclatement par parenthèses
  // internes. L'annotation finale n'alimente le poste QUE si c'est une vraie
  // fonction ("(Co-gérants)") ; une zone ("(Bretagne)") part en note partagée.
  const mFin = txt.match(/\(([^)]+)\)\s*$/)
  if (mFin && !aEclateParen) {
    const annot = mFin[1].trim()
    txt = txt.slice(0, mFin.index).trim()
    if (estFonction(annot)) { if (!posteShared) posteShared = annot }
    else noteShared = annot
  }
  ...
  for (let part of parts) {
    ...
    if (m) {
      ...
      // Fonction réelle -> poste ; zone/mention -> note_personne (poste vide).
      if (estFonction(annot)) poste = annot
      else note = annot
    ...
    gens.push({ prenom, nom, poste, civilite, note })
  }
  return gens
}
  ...
  for (const l of lignes) {
    ...
    gens.forEach((g, idx) => {
      ...
      const personne = {
        ...
        email, tel, linkedin, note_personne: g.note || '',
