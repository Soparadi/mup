...
// ── LES CLES D'APPARIEMENT D'UNE FICHE ─────────────────────────────────
// Le jeu de cles qu'un appariement par le nom compare au nom d'un candidat.
// QUATRE, depuis la mesure du 4 septembre 2026 sur 75 / 73.11Z et 15 / 74.20Z :
//
//   denomination       la raison sociale entiere
//   enseigne           le champ enseigne, quand il est rempli
//   hors-parentheses   la raison sociale privee de ses parentheses
//   parentheses        CE QUE LA DERNIERE PARENTHESE CONTIENT
//
// POURQUOI LA QUATRIEME. L'INSEE range l'enseigne DANS les parentheses de la
// raison sociale quand le champ enseigne est vide, et il l'est presque toujours :
// 198 fiches sur 6 164 au 75 / 73.11Z, 3 sur 58 au 15 / 74.20Z, la ou 1 502 et 33
// portent des parentheses. Aucune des trois premieres cles ne lit ce contenu
// ISOLEMENT : la denomination le noie dans le reste, hors-parentheses le retire.
// « PASCAL BESSE (PASCAL BESSE PHOTOGRAPHE) » contre « Pascal Besse Photographe »
// sortait ainsi a 0,780, sous le seuil ; la parenthese seule donne 1,000.
//
// CE QU'ELLE RAPPORTE, a seuil, rayon et survivant unique strictement inchanges :
//   75 / 73.11Z   196 appariements -> 208   (+13 gagnes, 1 perdu)
//   15 / 74.20Z     0 appariements ->   3   (+3 gagnes, 0 perdu)
// Quinze des seize gagnes sont a 1,000 et a moins de quinze metres. AUCUNE BASCULE
// sur aucun des deux couples : la quatrieme cle n'a jamais repris a son compte un
// appariement qu'une autre tenait deja, elle n'en ajoute que de nouveaux.
//
// LE SEUL PERDU dit ce qu'elle coute, et ce n'est pas un defaut d'elle :
// « MONSIEUR L' AGENT (ODDITY PARIS) » retenait la seule ligne « Monsieur L'Agent ».
// La parenthese rend « Oddity Paris » recevable elle aussi, a la meme adresse : deux
// lignes recevables, donc fiche ecartee par le survivant unique. Cette fiche porte
// reellement deux marques au meme endroit, et on ne remplit que le certain.
//
// LA DERNIERE PARENTHESE, PAS TOUTES : « YELLOWDOTCOMM AGENCY (YELLOWDOTCOMM
// AGENCY) (YDCA) » doit rendre « YDCA », que la source met en dernier. C'est la
// forme qui a ete mesuree, et la seule dont le rendement ci-dessus reponde.
//
// LA DEDUPLICATION N'EST PAS COSMETIQUE : deux cles de meme normalisation
// compteraient deux fois le meme candidat sans rien y ajouter, et « X (X) » est un
// cas courant. L'ordre de la liste est celui qui nomme l'origine du score retenu
// quand deux cles font jeu egal.
//
// PURE, sans I/O. Rend [{ origine, norm }], cles vides retirees.
export function clesAppariement(fiche) {
  const dedans = parentheses(fiche?.raison_sociale)
  const sources = [
    { origine: 'denomination', norm: normaliserSociete(fiche?.raison_sociale) },
    { origine: 'enseigne', norm: normaliserSociete(fiche?.enseigne) },
    { origine: 'hors-parentheses', norm: normaliserSociete(horsParentheses(fiche?.raison_sociale)) },
    { origine: 'parentheses', norm: normaliserSociete(dedans[dedans.length - 1] || '') }
  ]
  const vues = new Set()
  const out = []
  for (const s of sources) {
    if (!s.norm || vues.has(s.norm)) continue
    vues.add(s.norm)
    out.push(s)
  }
  return out
...
// ── LA GARDE DE GEOLOCALISATION ────────────────────────────────────────
// Dit si une fiche est REELLEMENT geolocalisee. Le filtre applicatif
// « lat != NONE AND lng != NONE » ne le dit pas : une fiche a lat 0 et lng 0 le
// passe sans etre situee nulle part. L ile nulle est au large du golfe de Guinee,
// a plus de quatre mille kilometres de la France.
//
// CE QUE CA COUTE, et c est la ce qui justifie la garde, pas la proprete. Mesure du
// 4 septembre 2026 sur le 15 / 74.20Z : DEUX fiches sur 63. Un tirage d appariement
// borne sa lecture a la boite englobante des fiches, decoupee en bandes de latitude ;
// ces deux fiches etiraient la boite depuis la latitude du Cantal jusqu a 0, soit
// 908 BANDES AU LIEU DE 17, et un quart d heure de tirage au lieu de vingt-trois
// secondes. Deux fiches sur soixante-trois payent le tirage entier.
//
// ELLE ECARTE, ELLE NE CORRIGE PAS. Une coordonnee absente ne se devine pas, et une
// fiche non geolocalisee n a rien a faire dans un appariement par proximite : elle
// n est a portee d aucun candidat, et elle ne peut que fausser la boite qui borne la
// lecture. L appelant la compte et la laisse ou elle est.
//
// PURE, sans I/O.
export function estGeolocalisee(fiche) {
  const lat = fiche?.lat
  const lng = fiche?.lng
...
// ── Helpers de chargement de source, relogés depuis atout-france.js ──────
// Deux gestes que tout chargeur de source répète : borner par département depuis
// un code postal, et tirer un domaine propre d'un site écrit à la main. Ils sont
// nés dans le chargeur Atout France et rge.js les lui empruntait, ce qui faisait
// dépendre un module de source d'un autre module de source sans rapport. Ils
// vivent ici, où ni l'un ni l'autre ne nomme sa source d'origine.
//
// DÉPLACÉS À L'IDENTIQUE, jamais réécrits : la colonne `domaine` de
// referentiel_rge et de referentiel_overture est écrite par normaliserSite, et
// 2,16 M de lignes la portent déjà sous cette règle exacte. Ni fusion avec
// normalizeUrl (mentions-legales.js), qui ne rend pas de domaine et ne retire pas
// le www, ni simplification de `valeur`, dont le « - » vaut vide.
