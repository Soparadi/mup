...
const ORDRE = {
  tel: ['museofile', 'finess', 'overture', 'rge', 'mentions_legales'],
  courriel: ['finess', 'overture', 'rge', 'mentions_legales'],
  site: ['atout_france', 'museofile', 'overture', 'rna_waldec', 'rge', 'mentions_legales'],
  social: ['overture', 'mentions_legales']
}

// ── CE QUI N ENTRE PLUS AU MASTER, ET N Y REVIENT PAS ──────────────────────────
// Decision de Ben, 24 septembre 2026 : l annuaire des avocats du CNB (fichier du 17
// juillet 2026 portant telephones et courriels, hors du modele officiel et retire de
// la publication) et le RPPS (rediffusion en principe interdite par la charte d
// utilisation) sont retires du socle. Leurs noms restent ICI, et ici seulement,
// pour que la fonte ECHOUE si l un revient : dans les sources, dans l ordre d un
// canal, ou dans une colonne d origine du master avant son ecriture. Le depot UP
// porte la meme liste dans packages/base-centrale/src/licences.ts, qui les refuse a
// l ecriture, au chargement et a la vente.
const ORIGINES_INTERDITES = ['avocats', 'cnb', 'CNB annuaire des avocats', 'rpps']
const interdite = (s) => ORIGINES_INTERDITES.some((o) => o.toLowerCase() === String(s ?? '').trim().toLowerCase())
for (const s of [...Object.keys(SOURCES), ...Object.values(ORDRE).flat()]) {
  if (interdite(s)) throw new Error(`origine interdite au master : ${s}`)
}
...
-- sous_rang separait les niveaux de cle de l annuaire des avocats, retire le 24
-- septembre 2026. Il vaut zero partout ; il reste dans l ordre du classement, sans
-- effet, pour ne pas changer le departage des autres sources.
...
// Chaque source porte en origine son nom court, celui du manifeste.
const bloc = (canal, t) => `
  ${t}.valeur     AS contact_${canal},
  ${t}.source     AS contact_${canal}_origine,
...
// LE GARDE DES ORIGINES INTERDITES, lu sur la table master avant que le parquet ne
// soit ecrit : un master qui porterait l une d elles n existe jamais sur disque.
const interditesSql = ORIGINES_INTERDITES.map((o) => `'${o.toLowerCase()}'`).join(', ')
duck(`COPY (SELECT count(*) AS n FROM master WHERE ${['tel', 'courriel', 'site', 'social']
  .map((c) => `lower(coalesce(contact_${c}_origine, '')) IN (${interditesSql})`).join(' OR ')})
  TO '${join(TRAVAIL, 'origines-interdites.json')}' (FORMAT JSON, ARRAY true);`)
const nInterdites = Number(litJson('origines-interdites.json')[0].n)
if (nInterdites) throw new Error(`${nInterdites} ligne(s) du master portent une origine interdite : le parquet n est pas ecrit`)
console.log('  origines interdites au master : 0')

duck(`SET preserve_insertion_order = false;
...
-- master.
--
-- L EFFET DU CRAWL, canal par canal, en trois sorts : ouverture, completion,
-- corroboration.
...
const manifeste = {
  ...
  vocabulaire: {
    ...
    cle: 'la colonne visee dans la table de source : referentiel_overture.cle, referentiel_rge.cle, referentiel_atout_france.cle, et pour les autres l identifiant de la source. Une exception nommee : le crawl des mentions legales y porte L ADRESSE DE LA PAGE LUE, par canal, ce qui permet de dire d ou vient chaque coordonnee (art. 14 RGPD).',
  ...
  origines_retirees: {
    decision: 'Ben, 24 septembre 2026',
    annuaire_des_avocats: 'CNB, fichier du 17 juillet 2026 portant telephones et courriels, hors du modele officiel et retire de la publication. Ne verse plus rien au master.',
    rpps: 'rediffusion en principe interdite par la charte d utilisation. Ne verse plus rien au master.',
    garde: 'la fonte echoue si l une de ces origines revient dans les sources, dans l ordre d un canal, ou dans une colonne d origine du master avant son ecriture.',
    noms_refuses: ORIGINES_INTERDITES
  },
  crawl_des_mentions_legales: {
    ...
    cle_par_canal: {
      ...
      forme: 'plusieurs pages sont jointes par barre verticale entouree d espaces.'
    },
