...
const SYNONYMES = {
  ...
  nom_complet: ['name', 'nom complet', 'full name', 'display name', 'contact', 'nom contact', 'contact nom', 'prenom nom', 'nom et prenom', 'contact name'],
  poste: ['poste', 'position', 'job title', 'title', 'fonction', 'titre', 'organization 1 title', 'intitule du poste', 'role'],
  societe: ['societe', 'company', 'entreprise', 'organisation', 'organization', 'organization 1 name', 'employeur', 'raison sociale', 'agence', 'etablissement', 'nom etablissement', 'enseigne'],
  ...
  tel: ['tel', 'telephone', 'phone', 'mobile', 'mobile phone', 'business phone', 'phone 1 value', 'numero', 'portable', 'tel 1', 'telephone portable', 'telephone mobile', 'gsm', 'phone number', 'tel mobile'],
  linkedin: ['linkedin', 'profil linkedin', 'lien linkedin', 'url linkedin'],
...
function trouverCanon(nh) {
  ...
  // Match par inclusion : "telephone portable principal" -> tel.
  for (const [canon, list] of Object.entries(SYNONYMES)) {
    if (canon === 'url') continue
...
function normaliserGrille(rows) {
  ...
  const nomEstSociete = source === 'prospection'
  const colKeys = hs.map(nh => {
    let key = trouverCanon(nh)
    if (REVERSE_ENTETES[nh] === 'url' || nh === 'url') key = 'url'
    if (nomEstSociete && (key === 'nom' || key === 'nom_complet')) key = 'societe'
    return key
  })
  const lignes = []
  for (let r = 1; r < rows.length; r++) {
    ...
    const acc = {
      ...
      email: '', tel: '', linkedin: '', site: '', adresse: '', ville: '', cp: ''
    }
    row.forEach((val, i) => {
      ...
      if (key && acc[key] !== undefined) { if (!acc[key]) acc[key] = v; return }
      sniff(acc, v)
    })
    ...
    lignes.push({
      ...
      linkedin: acc.linkedin, site: acc.site,
      adresse: acc.adresse, ville: acc.ville, cp: acc.cp, source
    })
  }
  return lignes
}
...
function attacherCoords(s, l) {
  for (const k of ['email', 'tel', 'site', 'linkedin', 'adresse', 'ville', 'cp']) {
    if (!s[k] && l[k]) s[k] = l[k]
  }
}
...
function construirePlan(lignes) {
  ...
  for (const l of lignes) {
    ...
    if (societeTierce) {
      ...
      ensureSociete(societes, cleSociete, l.societe, l.source)
    } else if (independant) {
