...
export async function sendWelcomeEmail(db, { ownerId, userEmail, userName, companyId, provider }) {
  ...
  // La clé porte le fournisseur : le callback Microsoft écrit __microsoft__, le
  // callback Google __google__. Un fournisseur en dur ici ne retrouverait jamais
  // les credentials de l'autre — d'où le paramètre, sans valeur par défaut.
  const recordId = `${ownerId}__${provider}__${String(userEmail).replace(/[^a-zA-Z0-9._-]/g, '_')}`
...
// Le corps d'un courriel est du contenu hostile par défaut : rien de ce qui
// suit ne renvoie du HTML. La partie text/plain est préférée ; à défaut, le
// texte est EXTRAIT de la partie HTML, balises comprises. La page affiche
// ensuite cette chaîne en texte, jamais en balisage.
...
// Parcourt l'arbre MIME renvoyé par Gmail et rend la première partie texte.
// À défaut, la première partie HTML, dont le texte est extrait.
function googlePartsText(payload) {
  const plain = []
  const html = []
  const walk = (part) => {
    if (!part) return
    ...
    if (data && type === 'text/plain') plain.push(decodeBase64Url(data))
    else if (data && type === 'text/html') html.push(decodeBase64Url(data))
    for (const child of part.parts || []) walk(child)
  }
  walk(payload)
  const joinedPlain = plain.join('\n').trim()
  if (joinedPlain) return joinedPlain
...
export async function getGoogleMessageBody(db, ownerId, email, messageId) {
  ...
  let detail
  try {
    detail = await gmail.users.messages.get({ userId: 'me', id: String(messageId), format: 'full' })
  } catch (err) {
  ...
  return {
    ...
    subject: h('Subject'),
    date: h('Date') || null,
    text: googlePartsText(data.payload),
    attachments: []
  }
}
