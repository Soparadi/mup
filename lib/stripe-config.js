...
// Source unique côté serveur : pricing-doctrine.js.
import { PLANS } from '../server/config/pricing-doctrine.js'
...
// Labels affichés dans les emails Resend. Dérivés de PLANS.
export const PLAN_LABELS = Object.fromEntries(
  Object.entries(PLANS).map(([slug, p]) => [slug, p.label])
)
export const PLAN_PRICES_DISPLAY = Object.fromEntries(
  Object.entries(PLANS).map(([slug, p]) => [
    slug,
    { monthly: `${p.priceMonthly} €`, annual: `${p.priceAnnualTotal} €` }
  ])
)
