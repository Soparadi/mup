...
function construirePlan(lignes) {
  ...
  for (const l of lignes) {
    // §2 — civilité en tête de colonne société : c'est un individu, jamais une
    // raison sociale. On la sort de la société et, à défaut d'autre source
    // personne, on la traite comme nom_complet (la civilité sera détachée).
    if (l.societe && extraireCivilite(l.societe).civilite) {
      if (!l.prenom && !l.nom && !l.nom_complet) l.nom_complet = l.societe
      l.societe = ''
    }
    ...
    // Repli : sans société-cible (indépendant ou aucune société), une coordonnée
    // « entreprise » ne se perd pas et reste sur la personne plutôt qu'être jetée.
    const telPremier = tels.personne[0] || (!societeTierce ? tels.societe[0] : '') || ''
    const emailPremier = emails.personne[0] || (!societeTierce ? emails.societe[0] : '') || ''
