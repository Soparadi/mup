...
export function deriveAppState(user, now = Date.now()) {
  ...
  // d'abonnement. Deux sources disjointes :
  //   - email en dur = dev garanti libre MÊME si la base déconne ;
  ...
  const BYPASS_EMAIL = 'dev@soparadi.com'
  if (user.email?.toLowerCase().trim() === BYPASS_EMAIL || user.bypass === true) return 'active'
