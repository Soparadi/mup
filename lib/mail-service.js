...
// ── Pièces jointes : une forme neutre, quatre vocabulaires ────────────────
// Les quatre voies d'envoi ne nomment pas les mêmes choses de la même façon.
// La forme neutre — { filename, content (base64), contentType, cid, inline } —
// est celle que produit lib/mail-signature.js ; chaque voie la traduit ici,
// dans le vocabulaire vérifié auprès de son fournisseur :
//
//   nodemailer   filename, content, encoding:'base64', contentType, cid
//                (la disposition « inline » est posée par la bibliothèque
//                elle-même dès qu'une image porte un cid — on ne l'écrit pas)
//   Resend       filename, content, contentType, contentId
//                (l'API REST les nomme content_type / content_id ; le SDK
//                Node fait la conversion — c'est déjà ce qu'emploie
//                server/services/email.js pour le logo transactionnel)
//   Graph        @odata.type, name, contentBytes, contentType, contentId,
//                isInline
//   Gmail        rien de tel : le message entier est du MIME brut, construit
//                plus bas par buildRfc2822
//
// RÈGLE COMMUNE : on traduit ce que la forme neutre décrit, et on laisse
// passer intact ce qu'elle ne décrit pas. Une pièce donnée en `path`, `href`
// ou `raw` ne rentre pas dans cette forme mais nodemailer et Resend la
// comprennent déjà — la restreindre serait perdre ce que ces voies savaient
// faire avant cette passe.

// Ramène une pièce à la forme neutre, ou null si elle n'y entre pas. `inline`
// exige un identifiant de contenu : une partie déclarée incorporée sans cid
// n'a rien qui puisse la référencer depuis le HTML, et s'afficherait en pièce
// jointe ordinaire chez le destinataire.
function pieceNormalisee(a) {
  if (!a || typeof a !== 'object') return null
  const base64 = Buffer.isBuffer(a.content)
    ? a.content.toString('base64')
    : (typeof a.content === 'string' ? a.content : '')
  if (!base64) return null
  const cid = a.cid || a.contentId || null
  const incorporee = a.inline === true || a.contentDisposition === 'inline'
  return {
    filename: a.filename || 'piece-jointe',
    base64,
    contentType: a.contentType || a.content_type || null,
    cid,
    inline: Boolean(cid) && incorporee
  }
}

function piecesNormalisees(attachments) {
  return (Array.isArray(attachments) ? attachments : [])
    .map(pieceNormalisee)
    .filter(Boolean)
}

// nodemailer — voie SMTP. `encoding: 'base64'` est ce qui dit à la
// bibliothèque que la chaîne n'est pas du texte : sans lui elle enverrait les
// caractères du base64 au lieu des octets qu'ils codent.
function piecesNodemailer(attachments) {
  if (!Array.isArray(attachments) || !attachments.length) return undefined
  return attachments.map(a => {
    const p = pieceNormalisee(a)
    if (!p) return a
    const piece = { filename: p.filename, content: p.base64, encoding: 'base64' }
    if (p.contentType) piece.contentType = p.contentType
    if (p.inline) piece.cid = p.cid
    return piece
  })
}

function piecesResend(attachments) {
  if (!Array.isArray(attachments) || !attachments.length) return undefined
  return attachments.map(a => {
    const p = pieceNormalisee(a)
    if (!p) return a
    const piece = { filename: p.filename, content: p.base64 }
    if (p.contentType) piece.contentType = p.contentType
    if (p.inline) piece.contentId = p.cid
    return piece
  })
}

// Graph — seules les pièces en ligne sont traduites. Cette voie n'a jamais
// transporté de pièce jointe ordinaire ; lui en faire porter une maintenant
// changerait ce que produit un envoi sans pièce en ligne, ce que cette passe
// s'interdit. Le jour où un appelant en produira, la branche s'ajoutera ici.
function piecesGraph(attachments) {
  return piecesNormalisees(attachments)
    .filter(p => p.inline)
    .map(p => ({
      '@odata.type': '#microsoft.graph.fileAttachment',
      name: p.filename,
      contentType: p.contentType || 'application/octet-stream',
      contentBytes: p.base64,
      contentId: p.cid,
      isInline: true
    }))
}
...
async function sendViaResend(userId, from, { to, subject, body, html, attachments }) {
  ...
  // Le mappage perdait l'identifiant de contenu et le type : une image en
  // ligne arrivait donc en pièce jointe ordinaire, et le src="cid:…" du HTML
  // ne pointait sur rien. piecesResend porte les deux.
  const pieces = piecesResend(attachments)
...
export async function sendOne(db, userId, { to, subject, body, html, from_email, attachments }) {
  ...
  try {
    const info = await transport.sendMail({
      ...
      text: body || undefined,
      html: html || undefined,
      attachments: piecesNodemailer(attachments)
    })
...
// RFC 2045 borne à 76 caractères la ligne d'un corps encodé en base64. Sans
// ce repli, la pièce part sur une seule ligne de plusieurs centaines de
// milliers de caractères, que des serveurs refusent.
function replieBase64(s) {
  const net = String(s).replace(/[^A-Za-z0-9+/=]/g, '')
  const lignes = []
  for (let i = 0; i < net.length; i += 76) lignes.push(net.slice(i, i + 76))
  return lignes.join('\r\n')
}

// Un nom de fichier et un identifiant de contenu partent dans des en-têtes.
// Un retour à la ligne ou un guillemet y ouvrirait un en-tête de plus, choisi
// par qui a fourni la pièce : ils sont retirés avant écriture.
function nomMime(s) {
  return String(s || 'piece-jointe').replace(/[\r\n"\\]/g, '').slice(0, 120) || 'piece-jointe'
}
function cidMime(s) {
  return String(s || '').replace(/[\r\n<>"\s]/g, '')
}

// Les parties de l'alternative texte/HTML, telles qu'elles étaient écrites
// jusqu'ici. Extraites sans être touchées : elles servent maintenant à deux
// endroits — seules quand le message n'a pas de pièce en ligne, et à
// l'intérieur du multipart/related quand il en a une.
function partiesAlternative(body, html, boundary) {
  const parts = []
  if (body) {
    parts.push(`--${boundary}`)
...
// Encode RFC 2822 → base64url pour gmail.users.messages.send. Gmail n'a pas de
// champ de pièce jointe : il prend le message entier en MIME, et c'est donc ici
// que la pièce en ligne doit être bâtie.
//
// Une pièce en ligne présente, l'enveloppe devient un multipart/related
// (RFC 2387) dont la première partie est l'alternative texte/HTML inchangée et
// les suivantes les images, chacune portant son Content-ID. Aucune pièce en
// ligne, la construction est exactement celle d'avant cette passe — mêmes
// en-têtes, mêmes parties, mêmes octets.
//
// Les pièces qui ne sont pas en ligne restent ignorées sur cette voie, comme
// elles l'étaient : les porter changerait ce que produit un envoi sans pièce
// en ligne. Rien n'en produit aujourd'hui.
function buildRfc2822({ from, to, subject, body, html, attachments }) {
  const headers = []
  ...
  const enLigne = piecesNormalisees(attachments).filter(p => p.inline)

  let raw
  if (enLigne.length) {
    const bRel = 'mup_rel_' + Math.random().toString(36).slice(2, 12)
    const bAlt = 'mup_' + Math.random().toString(36).slice(2, 12)
    // Le paramètre `type` de RFC 2387 nomme le type de la partie racine —
    // celle que le client doit afficher, les autres n'étant là que pour
    // qu'elle y renvoie.
    const typeRacine = html ? 'multipart/alternative' : 'text/plain'
    headers.push(`Content-Type: multipart/related; type="${typeRacine}"; boundary="${bRel}"`)
    const parts = []
    parts.push(`--${bRel}`)
    if (html) {
      parts.push(`Content-Type: multipart/alternative; boundary="${bAlt}"`)
      parts.push('')
      parts.push(...partiesAlternative(body, html, bAlt))
    } else {
      // Cas sans référent : le HTML est ce qui porte le src="cid:…". La
      // signature produit toujours une partie HTML quand elle a un logo, donc
      // ce chemin ne se rencontre pas par elle — il est écrit pour ne pas
      // perdre la pièce si un autre appelant s'y présentait.
      parts.push('Content-Type: text/plain; charset="UTF-8"')
      parts.push('Content-Transfer-Encoding: 8bit')
      parts.push('')
      parts.push(body || '')
    ...
    for (const p of enLigne) {
      const nom = nomMime(p.filename)
      parts.push(`--${bRel}`)
      parts.push(`Content-Type: ${p.contentType || 'application/octet-stream'}; name="${nom}"`)
      parts.push('Content-Transfer-Encoding: base64')
      parts.push(`Content-ID: <${cidMime(p.cid)}>`)
      parts.push(`Content-Disposition: inline; filename="${nom}"`)
      parts.push('')
      parts.push(replieBase64(p.base64))
    }
    parts.push(`--${bRel}--`)
    raw = headers.join('\r\n') + '\r\n\r\n' + parts.join('\r\n')
  } else if (html) {
    ...
    raw = headers.join('\r\n') + '\r\n\r\n' + partiesAlternative(body, html, boundary).join('\r\n')
  } else {
...
async function sendViaGoogle(db, cred, { to, subject, body, html, attachments }) {
  const auth = await ensureFreshGoogleClient(db, cred)
  const gmail = google.gmail({ version: 'v1', auth })
  const raw = buildRfc2822({ from: cred.email, to, subject, body, html, attachments })
...
async function sendViaMicrosoft(db, cred, { to, subject, body, html, attachments }) {
  ...
  // La collection ne figure dans le message que s'il y a quelque chose à y
  // mettre : un tableau vide serait une différence de plus avec ce que cette
  // voie envoyait jusqu'ici, pour rien.
  const pieces = piecesGraph(attachments)
  if (pieces.length) message.attachments = pieces
  ...
  for (const m of messages) {
    try {
      ...
      enveloppes.push({
        ...
        to: h('To'),
        subject: h('Subject'),
        // Gmail rend son extrait échappé en entités HTML : « L&#39;adresse ».
        // La liste l'affiche en texte, jamais en balisage — l'entité doit donc
        // être résolue ici, comme le corps l'est déjà par htmlToText. Seule voie
        // à produire un extrait : IMAP et Resend rendent une chaîne vide, rien
        // ne repasse deux fois.
...
// « image/png; charset=... » → « image/png ». Le paramètre ne dit rien du type
// et la casse est celle qu'a écrite l'expéditeur : la liste blanche se compare
// sur un type nu, quelle que soit la voie qui l'a lu.
function typeNu(contentType) {
...
function imagesEnLigneOf(parsed) {
  ...
  for (const partie of parsed?.attachments || []) {
    ...
    if (!cid) continue
    const type = typeNu(partie.contentType)
...
// ── Lecture Gmail : la taille du message décide de la forme demandée ──
//
// Le format 'raw' rapatrie le message ENTIER, pièces jointes comprises. C'est
// ce qu'il faut pour les images en ligne, dont 'full' ne donne que des
// attachmentId, mais c'est ruineux dès que le message porte autre chose : un
// courriel avec une pièce de vingt-cinq mégaoctets se téléchargerait en entier
// parce que l'abonné l'a ouvert, pour n'en afficher que le texte.
//
// Le seuil est posé à 4 Mo, soit le double du cumul autorisé pour les images en
// ligne (IMAGES_EN_LIGNE_CUMUL_MAX). Ce facteur deux n'est pas décoratif : dans
// le message, ces octets sont encodés en base64 et pèsent un tiers de plus, et
// il reste à loger le texte et les en-têtes. Un message dont les images
// remplissent tout le budget passe donc encore par la voie brute ; au-delà,
// aucune image supplémentaire ne pourrait de toute façon entrer, et tout ce qui
// serait descendu en plus serait descendu pour être jeté.
//
// L'autre borne est la mémoire : 'raw' rend le message en base64, un tiers de
// plus encore sur le fil, et il faut ensuite le décoder puis le parser. Un
// plafond de 4 Mo tient l'appel sous ~15 Mo de pointe, ce que l'instance
// supporte ; 25 Mo n'y tiendraient pas.
const GMAIL_TAILLE_MAX_BRUT = 4 * 1024 * 1024

// Ce que le repli 'full' laisse entrer dans la réponse, par forme. Le repli
// existe pour les gros messages : il n'a pas à en recracher le poids.
const GMAIL_TEXTE_MAX = 1024 * 1024
...
export async function getGoogleMessageBody(db, ownerId, email, messageId) {
  ...
  // La taille est connue AVANT le téléchargement : le format 'minimal' ne rend
  // ni corps ni parties, seulement des compteurs — quelques centaines d'octets.
  // C'est lui qui décide de la suite, plutôt que de tout rapatrier pour en
  // jeter la plus grande part.
  let taille = 0
  try {
    const sonde = await gmail.users.messages.get({ userId: 'me', id, format: 'minimal' })
    taille = Number(sonde.data?.sizeEstimate) || 0
  } catch (err) {
    if (err?.code === 404 || err?.response?.status === 404) throw mailError('Message introuvable', 'not_found')
    throw err
  }
  // Taille inconnue : on ne parie pas dans le sens du téléchargement.
  const brut = taille > 0 && taille <= GMAIL_TAILLE_MAX_BRUT

  let detail
  try {
    // 'raw' et non 'full' quand la taille le permet : 'full' ne rend PAS les
    // octets des parties en ligne — pour elles il ne donne qu'un attachmentId,
    // qu'il faudrait aller chercher par une requête de plus, et une par image.
    // Le message brut arrive entier en une seule fois, et simpleParser en tire
    // les mêmes parties que la voie IMAP : une seule mécanique pour les deux
    // voies.
    detail = await gmail.users.messages.get({ userId: 'me', id, format: brut ? 'raw' : 'full' })
  } catch (err) {
    if (err?.code === 404 || err?.response?.status === 404) throw mailError('Message introuvable', 'not_found')
    throw err
  }
  const data = detail.data || {}
  if (!brut) return corpsGmailFull(data)
...
// Le repli, au-delà du seuil : le texte du message et son HTML d'origine, tirés
// de l'arbre des parties que rend 'full'. Les images en ligne ne suivent pas —
// leurs octets ne sont pas dans cette réponse et chacune coûterait une requête.
// Le HTML garde donc ses src="cid:", que la page ne saura pas résoudre : le
// message s'affiche, ses logos non. C'est le prix assumé pour ne pas
// télécharger vingt-cinq mégaoctets à l'ouverture.
function corpsGmailFull(data) {
  const payload = data.payload || {}
  const corps = { text: '', html: '' }
  collecteTextesGmail(payload, corps, 0)
  const enteteDate = enteteGmail(payload, 'date')
  const dateEntete = enteteDate ? new Date(enteteDate) : null
  const dateValide = dateEntete && !Number.isNaN(dateEntete.getTime())
    ? dateEntete
    : (data.internalDate ? new Date(Number(data.internalDate)) : null)
  return {
    id: data.id,
    folder: (data.labelIds || []).includes('SENT') ? 'sent' : 'inbox',
    from: enteteGmail(payload, 'from'),
    to: enteteGmail(payload, 'to'),
    subject: enteteGmail(payload, 'subject'),
    date: dateValide && !Number.isNaN(dateValide.getTime()) ? dateValide.toISOString() : null,
    text: corps.text || (corps.html ? htmlToText(corps.html) : ''),
    html: corps.html,
    attachments: [],
    inlineImages: []
  }
...
// Les en-têtes de 'full' arrivent en liste, et leur casse est celle que
// l'expéditeur a écrite : la comparaison se fait en minuscules.
function enteteGmail(payload, nom) {
  const cherche = nom.toLowerCase()
  for (const h of payload?.headers || []) {
    if (String(h?.name || '').toLowerCase() === cherche) return String(h?.value || '')
  }
  return ''
}

// Marche l'arbre des parties pour en tirer le premier text/plain et le premier
// text/html — ceux d'un multipart/alternative, dans l'ordre où le message les
// pose. Seules les parties dont les octets sont DÉJÀ là (body.data) sont lues :
// passé une certaine taille Gmail ne rend qu'un attachmentId, et aller le
// chercher serait exactement le téléchargement qu'on refuse ici.
function collecteTextesGmail(partie, sortie, profondeur) {
  if (!partie || profondeur > 20) return
  if (sortie.text && sortie.html) return
  const type = String(partie.mimeType || '').toLowerCase().split(';')[0].trim()
  const donnees = partie.body?.data
  // Un nom de fichier fait de la partie une pièce jointe, fût-elle du texte :
  // elle n'est pas le corps du message et n'a pas à s'afficher à sa place.
  const piece = !!String(partie.filename || '').trim()
  if (donnees && !piece && (type === 'text/plain' || type === 'text/html')) {
    const texte = texteGmailDecode(donnees, partie)
    if (type === 'text/html') { if (!sortie.html) sortie.html = texte }
    else if (!sortie.text) sortie.text = texte
  }
  for (const enfant of partie.parts || []) collecteTextesGmail(enfant, sortie, profondeur + 1)
}

// Sur la voie brute, mailparser décode tous les jeux de caractères. Ici il n'y
// a pas de message à lui donner, et le décodage se fait à la main : UTF-8 pour
// ce que Buffer sait nommer, latin1 pour le reste. Un jeu multi-octets exotique
// rendrait donc un texte imparfait — c'est le repli d'un repli, réservé aux
// messages assez gros pour l'avoir déclenché, et un texte approché vaut mieux
// qu'un cadre vide.
function texteGmailDecode(donnees, partie) {
  const octets = Buffer.from(String(donnees), 'base64url')
  const coupe = octets.length > GMAIL_TEXTE_MAX ? octets.subarray(0, GMAIL_TEXTE_MAX) : octets
  // mimeType est nu chez Gmail — « text/html », sans paramètre. Le jeu de
  // caractères est resté dans l'en-tête Content-Type de la partie.
  const contentType = enteteGmail(partie, 'content-type')
  const jeu = (/charset\s*=\s*"?([^";]+)"?/i.exec(contentType)?.[1] || '').toLowerCase().trim()
  if (!jeu || jeu === 'utf-8' || jeu === 'utf8' || jeu === 'us-ascii' || jeu === 'ascii') return coupe.toString('utf8')
  return coupe.toString('latin1')
}
...
// ── Lecture Microsoft Graph (Reçus / Envoyés) ──
// Même contrat de sortie que les deux autres voies : enveloppes seules pour la
// liste, corps au clic, rien d'écrit en base. Ce qui change tient au fournisseur.
//
// Les dossiers portent leur NOM CANONIQUE Graph — 'inbox', 'sentitems' — qui
// vaut quelle que soit la langue de la boîte. Pas de découverte à la manière
// d'IMAP : il n'y a rien à deviner, et donc pas de dossier « Envoyés »
// introuvable.
const GRAPH_BASE = 'https://graph.microsoft.com/v1.0'
const DOSSIERS_GRAPH = { inbox: 'inbox', sent: 'sentitems' }

// fetch n'a pas de délai propre : sans AbortController, un appel Graph qui ne
// répond pas pend indéfiniment. La borne est celle de la socket IMAP — une
// lecture ne pend pas plus longtemps ici que là. L'envoi (sendViaMicrosoft)
// garde le sien : il n'est pas touché par cette passe.
const GRAPH_TIMEOUT = IMAP_SOCKET_TIMEOUT
const GRAPH_MAX_MESSAGES = IMAP_MAX_MESSAGES

// La route classe sur err.mailKind, comme le pose withImapClient. Le message
// porté ici ne sert qu'aux journaux : sendMailReadError ne rend à l'abonné que
// ses propres phrases pour 'auth' et 'network'. Rien de Graph ne remonte.
function graphError(message, kind) {
  const err = new Error(message)
  err.mailKind = kind
  return err
}

// Un appel Graph, borné dans le temps et traduit en erreur classable.
//   404 → 'not_found' : le message n'est plus là, ce n'est ni la boîte ni le
//         réseau. La route rend 404 et la page le dit.
//   401/403 → 'auth' : jeton refusé ou consentement trop étroit. Les deux se
//         réparent en reconnectant la boîte — c'est ce que 'auth' propose.
//   le reste (429, 5xx, socket tombée, délai dépassé) → 'network' : on invite à
//         réessayer, ce qui a une chance d'aboutir.
async function appelGraph(jeton, url, tag) {
  const arret = new AbortController()
  const minuteur = setTimeout(() => arret.abort(), GRAPH_TIMEOUT)
  try {
    ...
    if (!resp.ok) {
      ...
      try {
        ...
        detail = corps?.error?.message || detail
      } catch (e) { /* pas de corps JSON */ }
      console.error(tag, 'Graph a répondu', resp.status, '—', detail)
      if (resp.status === 404) throw mailError('Message introuvable', 'not_found')
      if (resp.status === 401 || resp.status === 403) throw graphError('Graph refuse l\'accès (' + resp.status + ')', 'auth')
      throw graphError('Graph a répondu ' + resp.status, 'network')
    }
    return await resp.json()
  } catch (err) {
    if (err?.mailKind || err?.code) throw err
    console.error(tag, 'appel Graph échoué —', err?.name, err?.message)
    throw graphError(err?.name === 'AbortError' ? 'Délai dépassé sur Graph' : 'Graph injoignable', 'network')
  } finally {
    clearTimeout(minuteur)
  }
}

async function credentialMicrosoft(db, ownerId, email) {
  const cred = await loadMailboxCredential(db, ownerId, email)
  if (!cred || cred.provider !== 'microsoft') {
    throw mailError('Compte Microsoft introuvable pour cet utilisateur', 'no_account')
  }
  return cred
}

// Le jeton frais, et son échec classé. Un refresh_token révoqué, périmé ou
// illisible se répare en reconnectant la boîte : c'est 'auth'. Seule une
// panne de transport vers l'autorité Microsoft vaut 'network' — inviter à
// réessayer quand le consentement est mort n'aboutirait jamais.
async function jetonMicrosoft(db, cred) {
  try {
    return await ensureFreshMicrosoftToken(db, cred)
  } catch (err) {
    const transport = err?.name === 'TypeError' ||
      /fetch failed|ECONNRESET|ECONNREFUSED|ETIMEDOUT|EAI_AGAIN|socket hang up/i.test(String(err?.message || ''))
    console.error('[graph:token] rafraîchissement échoué —', err?.message)
    throw graphError('Jeton Microsoft non renouvelable', transport ? 'network' : 'auth')
  }
}

// Les adresses de Graph arrivent en { emailAddress: { name, address } }. Une
// fois ramenées à la forme des enveloppes IMAP, c'est formatImapAddress qui
// les rend — même troncature à trois, même « +N », une seule mise en forme
// pour toutes les voies.
function adressesGraph(liste) {
  if (!Array.isArray(liste)) return ''
  return formatImapAddress(liste.map(r => ({
    name: r?.emailAddress?.name || '',
    address: r?.emailAddress?.address || ''
  })))
}

function adresseGraph(recipient) {
  return recipient?.emailAddress ? adressesGraph([recipient]) : ''
}

// ISO 8601, ou null. Graph rend déjà de l'ISO en UTC ; le passage par Date
// écarte ce qui n'en serait pas et aligne la forme sur la voie IMAP. Le premier
// champ renseigné gagne : le dossier décide lequel est demandé en premier.
function dateGraph(...valeurs) {
...
// Cinquante messages au plus, les plus récents en tête, sans fenêtre de date :
// le dossier est lu tel quel. $select borne la réponse aux champs de
// l'enveloppe — le corps ne descend jamais par la liste.
export async function listMicrosoftMessages(db, ownerId, email, { folder = 'inbox', limit = GRAPH_MAX_MESSAGES } = {}) {
  const voulu = folder === 'sent' ? 'sent' : 'inbox'
  const nombre = Math.min(Math.max(Number(limit) || GRAPH_MAX_MESSAGES, 1), GRAPH_MAX_MESSAGES)
  const cred = await credentialMicrosoft(db, ownerId, email)
  const jeton = await jetonMicrosoft(db, cred)
  const champs = 'id,subject,bodyPreview,from,toRecipients,receivedDateTime,sentDateTime,isRead'
  // Un message envoyé n'a pas été reçu : le dossier décide de la date qui trie
  // comme de celle qui s'affiche.
  const tri = voulu === 'sent' ? 'sentDateTime desc' : 'receivedDateTime desc'
  const url = `${GRAPH_BASE}/me/mailFolders/${DOSSIERS_GRAPH[voulu]}/messages`
    + `?$top=${nombre}&$select=${champs}&$orderby=${encodeURIComponent(tri)}`
  const data = await appelGraph(jeton, url, '[graph:list]')
  return (data?.value || []).map(m => ({
    id: String(m?.id || ''),
    from: adresseGraph(m?.from),
    to: adressesGraph(m?.toRecipients),
    subject: m?.subject || '',
    // bodyPreview est le début du corps EN TEXTE, tel que Graph le rend à la
    // liste : aucun appel de plus, et les entités résolues comme sur la voie
    // Gmail — la liste l'affiche en texte, jamais en balisage.
    snippet: decodeEntities(m?.bodyPreview || ''),
    date: voulu === 'sent'
      ? dateGraph(m?.sentDateTime, m?.receivedDateTime)
      : dateGraph(m?.receivedDateTime, m?.sentDateTime),
    unread: m?.isRead === false,
    folder: voulu
  }))
}

// Une référence cid: dans le HTML, et rien d'autre, déclenche la recherche des
// parties en ligne. Sans elle, aucune image ne serait affichable : l'appel
// serait payé pour rien. hasAttachments ne sert pas d'indice — Graph le met à
// faux quand un message ne porte QUE des pièces en ligne.
const REFERENCE_CID = /=\s*["']?\s*cid:/i

// Miroir de imagesEnLigneOf, mêmes bornes exactement — liste blanche de types,
// svg refusé, 512 Ko par partie, 2 Mo cumulés, 20 au plus. La différence tient
// au fournisseur : sur les voies IMAP et Gmail les octets sont déjà là, ici
// Graph ne les rend qu'à la demande et la collection complète les descendrait
// TOUS. On lit donc d'abord les seules métadonnées ($select sans contentBytes),
// on écarte ce qui ne peut pas être une image en ligne, et on ne télécharge que
// ce qui reste.
//
// Le pré-filtre sur `size` laisse le double du plafond : cette taille est celle
// de la pièce dans le magasin Exchange, encodage compris, et non celle des
// octets décodés. La borne qui fait foi reste mesurée après décodage, comme
// ailleurs. Une image en ligne ne coûte donc jamais plus d'un mégaoctet
// descendu, ni plus que le cumul autorisé une fois décodée.
async function imagesEnLigneGraph(jeton, messageId, tag) {
  const base = `${GRAPH_BASE}/me/messages/${encodeURIComponent(messageId)}/attachments`
  const liste = await appelGraph(jeton, base + '?$select=id,name,contentType,size,isInline', tag)
  const candidates = (liste?.value || [])
    .filter(p => TYPES_IMAGE_EN_LIGNE.has(typeNu(p?.contentType)))
    .filter(p => Number(p?.size) > 0 && Number(p?.size) <= IMAGE_EN_LIGNE_MAX * 2)
    // Ce qu'Exchange marque lui-même comme en ligne passe en premier : c'est à
    // ces parties-là que le budget cumulé doit profiter d'abord. Les autres ne
    // sont pas écartées pour autant — un contentId suffit à référencer une
    // partie, et c'est lui qu'on vérifiera une fois la pièce descendue.
    .sort((a, b) => Number(b?.isInline === true) - Number(a?.isInline === true))
  const sorties = []
  let cumul = 0
  for (const meta of candidates) {
    if (sorties.length >= IMAGES_EN_LIGNE_NOMBRE_MAX) break
    const piece = await appelGraph(jeton, `${base}/${encodeURIComponent(String(meta?.id || ''))}`, tag)
    // Sans identifiant de contenu, la partie n'est référençable par aucun
    // cid: — c'est une pièce jointe ordinaire, elle ne nous regarde pas.
    const cid = String(piece?.contentId || '').replace(/^<+|>+$/g, '').trim()
    if (!cid) continue
    const type = typeNu(piece?.contentType)
    if (!TYPES_IMAGE_EN_LIGNE.has(type)) continue
    const octets = Buffer.from(String(piece?.contentBytes || ''), 'base64')
    if (!octets.length || octets.length > IMAGE_EN_LIGNE_MAX) continue
    if (cumul + octets.length > IMAGES_EN_LIGNE_CUMUL_MAX) break
    cumul += octets.length
    sorties.push({ cid, contentType: type, content: octets.toString('base64') })
  }
...
// Le corps d'UN message, au clic. Deux champs comme les autres voies : le texte
// pour le repli, le HTML d'origine non retouché — c'est la page qui l'enferme.
// La liste des pièces jointes part vide et aucun appel n'est fait pour elle :
// ce que rend la page aujourd'hui ne s'en sert pas.
export async function getMicrosoftMessageBody(db, ownerId, email, messageId, { folder = 'inbox' } = {}) {
  // Le dossier n'est pas dérivé du message : la valeur reçue est échouée telle
  // quelle, comme sur les deux autres voies. Graph sait retrouver un message
  // par son seul identifiant, quel que soit le dossier qui le porte.
  const voulu = folder === 'sent' ? 'sent' : 'inbox'
  const id = String(messageId || '').trim()
  if (!id) throw mailError('Identifiant de message invalide', 'not_found')
  const cred = await credentialMicrosoft(db, ownerId, email)
  const jeton = await jetonMicrosoft(db, cred)
  const champs = 'id,subject,body,from,toRecipients,receivedDateTime,sentDateTime'
  const m = await appelGraph(
    jeton,
    `${GRAPH_BASE}/me/messages/${encodeURIComponent(id)}?$select=${champs}`,
    '[graph:message]'
  )
  // body.contentType dit ce que contient body.content : 'html' ou 'text'. Un
  // message en texte seul n'a pas de partie HTML — la page retombe sur le
  // texte, exactement comme quand mailparser rend html à faux.
  const contenu = String(m?.body?.content || '')
  const html = typeNu(m?.body?.contentType) === 'html' ? contenu : ''
  let imagesEnLigne = []
  if (html && REFERENCE_CID.test(html)) {
    try {
      imagesEnLigne = await imagesEnLigneGraph(jeton, id, '[graph:inline]')
    } catch (err) {
      // Les images en ligne sont un supplément : leur échec ne doit pas
      // emporter le message. Il s'affiche, ses logos non.
      console.error('[graph:inline] parties en ligne non résolues —', err?.message)
    }
  }
  return {
    id: String(m?.id || id),
    ...
    date: voulu === 'sent'
      ? dateGraph(m?.sentDateTime, m?.receivedDateTime)
      : dateGraph(m?.receivedDateTime, m?.sentDateTime),
    text: html ? htmlToText(html) : contenu,
    html,
    attachments: [],
    inlineImages: imagesEnLigne
  }
}
